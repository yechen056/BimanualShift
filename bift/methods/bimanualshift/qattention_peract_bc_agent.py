...
from bift.helpers.utils import visualise_voxel, stack_on_channel
from bift.voxel import augmentation_ab
from bift.voxel.voxel_grid import VoxelGrid
from bift.voxel.augmentation import apply_se3_augmentation
from einops import rearrange
from bift.helpers.clip.core.clip import build_model, load_clip

import transformers
from bift.helpers.optim.lamb import Lamb
import wandb
...
ENABLE_TRAIN_DEBUG_VIS = os.environ.get("ANYBIMANUAL_DEBUG_TRAIN_VIS", "0") == "1"
...
class QFunction(nn.Module):
    ...
    def forward(
        ...
        if hasattr(self._qnet, "set_runtime_context"):
            self._qnet.set_runtime_context(rgb=rgb, lang=lang_goal_text)
        elif hasattr(self._qnet, "module") and hasattr(self._qnet.module, "set_runtime_context"):
            self._qnet.module.set_runtime_context(rgb=rgb, lang=lang_goal_text)
        split_pred, L_skill, L_voxel, mask_right, mask_left = self._qnet(
            voxel_grid,
            proprio,
            lang_goal_emb,
            lang_token_embs,
        ...
        return split_pred, voxel_grid, L_skill, L_voxel, mask_right, mask_left
...
class QAttentionPerActBCAgent(Agent):
    def __init__(
        ...
        memory_bank: SkillMemoryBank = None,
    ):
        self.frozen = frozen
        ...
        self.cfg = cfg
        self._use_memory_bank = use_memory_bank
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        ...
        lang_goal_text = replay_sample.get("lang_goal", None)
        ...
        if self._transform_augmentation:
            from bift.voxel import augmentation, augmentation_ab
            if self.aug_type == "ab":
                (
                    right_action_trans,
        ...
        # forward pass
        q, voxel_grid, L_skill, L_voxel, _, _ = self._q(
            obs,
            proprio,
            pcd,
            lang_goal_emb,
            lang_token_embs,
            lang_goal_text=lang_goal_text,
            bounds=bounds,
        ...
        if self._use_memory_bank and self._memory_bank is not None:
            reward = replay_sample.get("reward", None)
            if reward is not None and torch.max(reward).item() > 0:
                key = self._memory_bank.build_key(lang_goal_emb[0].detach(), voxel_grid[0:1].detach())
                act_vec = torch.cat(
                    [
                        right_action_trans[0].float(),
                        right_action_rot_grip[0].float(),
                        right_action_ignore_collisions[0].float(),
                        left_action_trans[0].float(),
                        left_action_rot_grip[0].float(),
                        left_action_ignore_collisions[0].float(),
                    ],
                    dim=0,
                )
                self._memory_bank.update(key, act_vec, success=True)

        if step % 10 == 0 and rank == 0 and self.cfg.framework.use_wandb:
            wandb.log({
        ...
        q_trans_vis = True
        if ENABLE_TRAIN_DEBUG_VIS and step % self.cfg.framework.log_freq == 0 and rank == 0:
    ...
    def act(self, step: int,observation: dict,deterministic=False) -> ActResult:
        ...
        lang_goal_text = observation.get("lang_goal", None)
        ...
        with torch.no_grad():
            ...
            (
                lang_goal_emb,
                lang_token_embs,
            ) = self._clip_rn50.encode_text_with_embeddings(lang_goal_tokens.squeeze(0))
        ...
        ), vox_grid, _, _, mask_right, mask_left = self._q(
            obs,
            proprio,
            pcd,
            lang_goal_emb,
        ...
        if self._use_memory_bank and self._memory_bank is not None:
            key = self._memory_bank.build_key(lang_goal_emb.detach().reshape(-1), vox_grid[0:1].detach())
            retrieved, sim = self._memory_bank.retrieve(key)
            if retrieved is not None:
                current_vec = torch.cat(
                    [
                        right_coords[0].float(),
                        right_rot_grip_action[0].float(),
                        right_ignore_collisions[0].float(),
                        left_coords[0].float(),
                        left_rot_grip_action[0].float(),
                        left_ignore_collisions[0].float(),
                    ],
                    dim=0,
                )
                refined = self._memory_bank.refine(current_vec, retrieved.to(current_vec.device))
                idx = 0
                right_coords[0] = refined[idx : idx + 3].round().long().clamp(0, self._voxel_size - 1)
                idx += 3
                right_rot_refined = refined[idx : idx + 4].round().long()
                right_rot_refined[:3] = right_rot_refined[:3].clamp(0, max_rot_index - 1)
                right_rot_refined[3] = right_rot_refined[3].clamp(0, 1)
                right_rot_grip_action[0] = right_rot_refined
                idx += 4
                right_ignore_collisions[0] = refined[idx : idx + 1].round().long().clamp(0, 1)
                idx += 1
                left_coords[0] = refined[idx : idx + 3].round().long().clamp(0, self._voxel_size - 1)
                idx += 3
                left_rot_refined = refined[idx : idx + 4].round().long()
                left_rot_refined[:3] = left_rot_refined[:3].clamp(0, max_rot_index - 1)
                left_rot_refined[3] = left_rot_refined[3].clamp(0, 1)
                left_rot_grip_action[0] = left_rot_refined
                idx += 4
                left_ignore_collisions[0] = refined[idx : idx + 1].round().long().clamp(0, 1)
                if hasattr(self, "_summaries") and isinstance(self._summaries, dict):
                    self._summaries["memory/retrieval_sim"] = float(sim)
        ...
        # if mask_right is not None and mask_left is not None:
        #     # 掩码的形状是 [B, 8000, 128]，我们需要将其转换回体素网格形状
        #     # 8000 = 20*20*20，所以 patch grid 的尺寸是 (20, 20, 20)
            
        #     # 处理右臂掩码
        #     mask_right_grid = mask_right[0].view(20, 20, 20, 128).mean(dim=-1) # 取特征维度的平均值
        #     mask_right_grid = mask_right_grid.unsqueeze(0).unsqueeze(0) # 增加 Batch 和 Channel 维度 -> [1, 1, 20, 20, 20]
            
        #     # 使用三线性插值将掩码从 patch grid (20^3) 上采样到完整的 voxel grid (100^3)
        #     upsampled_mask_right = F.interpolate(mask_right_grid, size=(100, 100, 100), mode='trilinear', align_corners=False)
        #     upsampled_mask_right = upsampled_mask_right.squeeze(0).cpu().detach().numpy() # [1, 100, 100, 100]
            
        #     rotation_angle = np.deg2rad(self.episode_step * 5)
        #     # 使用 visualise_voxel 函数渲染右臂的视觉掩码
        #     rendered_mask_right = visualise_voxel(
        #         vox_grid[0].cpu().detach().numpy(),
        #         #q_attention=upsampled_mask_right, # 将上采样后的掩码作为注意力热图
        #         highlight_coordinate=None,
        #         highlight_gt_coordinate=None, # 评估时没有GT
        #         rotation_amount=rotation_angle
        #     )

        #     # 处理左臂掩码
        #     mask_left_grid = mask_left[0].view(20, 20, 20, 128).mean(dim=-1)
        #     mask_left_grid = mask_left_grid.unsqueeze(0).unsqueeze(0)
        #     upsampled_mask_left = F.interpolate(mask_left_grid, size=(100, 100, 100), mode='trilinear', align_corners=False)
        #     upsampled_mask_left = upsampled_mask_left.squeeze(0).cpu().detach().numpy()

        #     # 渲染左臂的视觉掩码
        #     rendered_mask_left = visualise_voxel(
        #         vox_grid[0].cpu().detach().numpy(),
        #         #q_attention=upsampled_mask_left,
        #         highlight_coordinate=None,
        #         highlight_gt_coordinate=None, # 评估时没有GT
        #         rotation_amount=rotation_angle
        #     )
        ...
        #     # 将左右臂的掩码图并排显示
        #     fig_masks, axs_masks = plt.subplots(1, 2, figsize=(16, 8))
        #     axs_masks[0].imshow(rendered_mask_right)
        #     axs_masks[0].set_title('Right Arm Mask')
        #     axs_masks[0].axis('off')
            
        #     axs_masks[1].imshow(rendered_mask_left)
        #     axs_masks[1].set_title('Left Arm Mask')
        #     axs_masks[1].axis('off')
            
        #     plt.tight_layout()

        #     save_filename = f'eval_vis/ep{self.episode_id}_step{self.episode_step}_masks.png'
        #     plt.savefig(save_filename, dpi=600)
        #     cprint(f'Saved {save_filename} locally', 'cyan')
    ...
    def load_weights(self, savedir: str):
        ...
        if not self._training:
            ...
            for k, v in state_dict.items():
                ...
                if k in merged_state_dict and merged_state_dict[k].shape == v.shape:
                    merged_state_dict[k] = v
                else:
                    if "_voxelizer" not in k:
                        missing_checkpoint_keys.append(k)
            if missing_checkpoint_keys:
                preview = ", ".join(missing_checkpoint_keys[:5])
                suffix = " ..." if len(missing_checkpoint_keys) > 5 else ""
                logging.warning(
                    "Skipped %d checkpoint keys with no compatible target shape or name during eval load: %s%s",
            ...
            for k, v in state_dict.items():
                if not self._training:
                    k = k.replace("_qnet.module", "_qnet")
                # When resuming a bimanual checkpoint from the same code path,
                # prefer direct key matches and skip legacy remapping logic.
                if k in merged_state_dict and merged_state_dict[k].shape == v.shape:
                    merged_state_dict[k] = v
                    continue
                # cross_attn
                ...
                elif k in merged_state_dict and merged_state_dict[k].shape == v.shape:
                    merged_state_dict[k] = v
        ...
        if self.frozen:
            logging.info("Freezing parameters from PerAct")
            for name, param in self._q.named_parameters():
